...
class Model(object):
    ...
    collapse_chunks = True # Collapse chunks with identical slots and slot values (default in ACT-r)

    chunk_name_counter = 0
    ...
    def add_encounter(self, chunk):
        ...
        if chunk.name not in [chunk.name for chunk in self.dm]:
           # Check if there is already a chunk with the same slots and values
            found = False
            if self.collapse_chunks: # If we don't collapse chunks, we always add, so found is always false
                for c1 in self.dm:
                    if chunk.slots == c1.slots:
                        chunk = c1
                        found = True
                        break
            if not found:
    ...
    def new_name(self, string = "chunk"):
        self.chunk_name_counter += 1
        return string + str(self.chunk_name_counter)
